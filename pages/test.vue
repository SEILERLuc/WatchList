      ...
      <div class="mb-2 mt-12 flex flex-col items-center px-2">
        ...
        <div class="flex flex-wrap justify-center">
          ...
          >
            ...
            <label
              for="show"
              class="color-white sm:w-18 hover:rotate-10 flex h-8 w-16 items-center justify-center rounded-md border duration-200 sm:h-10"
              ><button type="button" @click="filterShow(genre)">
                {{ genre }}
              </button></label
            >
          </div>
        ...
        <div v-if="activeGenre !== 'All'" class="flex flex-row items-start">
          <h3 class="color-white mb-1 mt-2 py-2 text-lg font-bold sm:text-3xl">
            Filtered by {{ activeGenre }}
          </h3>
        </div>
        <div>
          <!--CARD LIST OF THE SHOWS-->
          <ul
            class="mb-1 mt-1 flex flex-wrap justify-center font-mono text-xs max-w-7xl"
          >
