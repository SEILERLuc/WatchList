<template>
  <div class="flex flex-col items-center my-4">
    <img src="/img/UI/home.png" class="w-8" alt="HOME PAGE" />
    <p class="text-justify my-4">
      In the Home page, you can research your own shows in the. When you tap a
      name, the app make a research to the API and show you the returned
      response. There is a
      <i><b>limit result of 10</b></i
      >.
    </p>
    <p class="text-justify">
      Then, you can choose to find more informations by clicking the show. You
      can access to the title, the rating and the summary. You can choose to add
      the show to your list too, by clicking the button.
    </p>
  </div>
</template>
